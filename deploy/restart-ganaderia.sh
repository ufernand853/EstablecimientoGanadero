...
WEB_HEALTH_URL="http://127.0.0.1:${WEB_PORT}/api/proxy/health"
...
wait_for_port_release() {
  ...
  for ((i=1; i<=attempts; i++)); do
    if ! ss -tulpn 2>/dev/null | grep -q ":${port} "; then
      echo "$label liberado: $port"
      return 0
    fi
    echo "Esperando liberacion de $label en puerto $port ($i/$attempts)..."
    sleep "$sleep_seconds"
  done

  echo "No se pudo liberar $label en puerto $port"
  return 1
}
...
wait_for_port_release "$WEB_PORT" "web" || true
wait_for_port_release "$API_PORT" "API" || true
