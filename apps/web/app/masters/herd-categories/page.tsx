...
export default function HerdCategoriesPage() {
  ...
  useEffect(() => {
    load().catch(() => setError("No se pudieron cargar categorías."));
  }, []);
  ...
  const handleCreate = async (event: FormEvent) => {
    ...
    if (!response.ok) {
      ...
      setError(data?.message ?? "No se pudo crear categoría.");
      return;
    }
    setName("");
  ...
  return (
    <main className="space-y-6">
      <header className="flex items-center justify-between">
        <h2 className="text-xl font-semibold">Categorías de ganado</h2>
      ...
      <section className="rounded-lg bg-slate-900 p-4">
        <form className="flex gap-3" onSubmit={handleCreate}>
          <input className="flex-1 rounded bg-slate-800 p-2 text-sm" placeholder="Ej: TOROS" value={name} onChange={(e) => setName(e.target.value.toUpperCase())} />
      ...
      <section className="rounded-lg bg-slate-900 p-4 grid gap-2">
        ...
        {categories.length === 0 && <p className="text-sm text-slate-400">Sin categorías.</p>}
      </section>
    </main>
  );
}
